    ...
    [afn getDataFromPath:METHOD_GETPROFILEMATCHES withParamData:dictParam withBlock:^(id response, NSError *error) {
        if (response) {
            if ([[response objectForKey:@"errFlag"] intValue]==0) {
                ...
                [[User currentUser] setNumberOfFriends:arr.count];
...
//    [[NSNotificationCenter defaultCenter] postNotificationName:@"chatVCforSpark" object:dict];
    ...
    [afn getDataFromPath:METHOD_GETPROFILEMATCHES withParamData:dictParam withBlock:^(id response, NSError *error) {
        if (response) {
            if ([[response objectForKey:@"errFlag"] intValue]==0) {
                ...
                {
                    if ([[dict objectForKey:@"flag"] integerValue]!=4)
                    {
                ...
                [[User currentUser] setNumberOfFriends:arrAllMatch.count];
                
                [self.tblView reloadData];
                
                NSString *status =[[NSUserDefaults standardUserDefaults] rm_customObjectForKey:@"ChatRedirect"];
                if ([status isEqualToString:@"1"]) {
                    [[NSUserDefaults standardUserDefaults] rm_setCustomObject:@"0" forKey:@"ChatRedirect"];
                    NSMutableDictionary *dict =[[NSUserDefaults standardUserDefaults] rm_customObjectForKey:@"chatVC"];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSPredicate *predict=[NSPredicate predicateWithFormat:@"senderId = %@ OR receiverId = %@",user.fbid,user.fbid];

...
//    NSString *dateString = [format stringFromDate:d];
    ...
    NSDateComponents* ageComponents = [[NSCalendar currentCalendar]
                                       components:NSDayCalendarUnit |NSHourCalendarUnit | NSMinuteCalendarUnit
                                       fromDate:dateFinal
                                       toDate:now
                                       options:0];
    NSString *days=@"";
    if (ageComponents.day > 0) {
        if (ageComponents.day > 1) {
            days = [NSString stringWithFormat:@"%d Days",ageComponents.day];
        }else{
            days = [NSString stringWithFormat:@"%d Day",ageComponents.day];
        }
    }

    
    NSString *hours=@"";
    if (ageComponents.hour > 0) {
        if (ageComponents.hour > 1) {
            hours = [NSString stringWithFormat:@"%li Hours",ageComponents.hour];
        }else{
            hours = [NSString stringWithFormat:@"%li Hour",ageComponents.hour];
        }
    }
    
    NSString *min=@"";
    if (ageComponents.minute > 0) {
        if (ageComponents.minute > 1) {
            min = [NSString stringWithFormat:@"%li Minutes",ageComponents.minute];
        }else{
            min = [NSString stringWithFormat:@"%li Minute",ageComponents.minute];
        }
    }else{
         min = [NSString stringWithFormat:@"%li Minute",ageComponents.minute];
    }
    NSString * stringDue = [NSString stringWithFormat:@"%@ %@ %@",days, hours,min];
    return stringDue;
}

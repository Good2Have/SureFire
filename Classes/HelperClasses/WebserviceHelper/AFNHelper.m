...
-(void)getDataFromPath:(NSString *)path withApiUrl:(NSString *)apiUrl withParamData:(NSMutableDictionary *)dictParam withBlock:(RequestCompletionBlock)block
{
    if (block)
    {
        dataBlock=[block copy];
    }
    NSURL *baseURL = [NSURL URLWithString:[NSString stringWithFormat:@"%@",apiUrl]];
    client = [[AFHTTPClient alloc] initWithBaseURL:baseURL];
    [client registerHTTPOperationClass:[AFJSONRequestOperation class]];
    [client setDefaultHeader:@"Accept" value:@"application/json"];
    //[client setDefaultHeader:@"Accept" value:@"application/x-www-form-urlencoded"];
    
    [client postPath:[NSString stringWithFormat:@"%@%@",apiUrl,path]
          parameters:dictParam
             success:^(AFHTTPRequestOperation *operation, id responseObject) {
                 DLog(@"response :%@",responseObject);
                 if (dataBlock)

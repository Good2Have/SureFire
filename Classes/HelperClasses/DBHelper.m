...
-(void)insertMsgToDB:(NSMutableArray *)arrMsgs uniqueId:(NSString *)uniqueId
{
    if (arrMsgs)
    {
        ...
        for (NSDictionary *dict in arrMsgs)
        {
            ...
            [self deleteDuplicateMsgFromDB:dict[@"mid"]];
            [self saveContext];
        }
    }
}
-(void)deleteMsgFromDB:(NSString *)uniqueId
{
    ...
    NSPredicate *predicate=[NSPredicate predicateWithFormat:@"receiverId == %@ OR senderId == %@",uniqueId,uniqueId];
    
            NSMutableArray *arr=[self getObjectsforEntity:ENTITY_MESSAGETABLE ShortBy:nil isAscending:NO predicate:predicate];
            for (MessageTable *msg in arr)
            {
...
-(void)deleteDuplicateMsgFromDB:(NSString *)MessageID
{

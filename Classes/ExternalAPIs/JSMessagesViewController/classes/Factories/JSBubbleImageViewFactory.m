...
//    UIImage *bubble = [UIImage imageNamed:@"bubble-min"];
    UIImage *bubble = [UIImage imageNamed:@"bg_chat_sender.png"];
    
    UIImage *normalBubble = bubble;// = [bubble js_imageMaskWithColor:color];
    UIImage *highlightedBubble = [bubble js_imageMaskWithColor:[color js_darkenColorWithValue:0.12f]];
    
    if(type == JSBubbleMessageTypeIncoming) {
        normalBubble = [UIImage imageNamed:@"bg_chat_reciever.png"];//[bubble js_imageFlippedHorizontal];
//        highlightedBubble = [highlightedBubble js_imageFlippedHorizontal];
    ...
    UIEdgeInsets capInsets = UIEdgeInsetsMake(30, 20, 20, 15);//UIEdgeInsetsMake(15.0f, 20.0f, 15.0f, 20.0f);
